...
public class SXPBlueprintSearchRequestEnhancerTest {
	...
	public static void setUpClass() throws PortalException {
		_sxpElement = _createCustomSXPElement();
	}
	...
	public void testBoostAClause() throws Exception {
		_addCustomSXPElementToSXPBlueprint();
	...
	private static SXPElement _createCustomSXPElement() throws PortalException {
		return _sxpElementLocalService.addSXPElement(
			TestPropsValues.getUserId(),
			Collections.singletonMap(LocaleUtil.US, "Custom Element"),
			_createElementDefinitionJSON(), false, "",
			Collections.singletonMap(LocaleUtil.US, "Custom Element"), 1,
			ServiceContextTestUtil.getServiceContext());
	}

	private static String _createElementDefinitionJSON() {
		ArrayList<JSONObject> mustNot = new ArrayList<>(
			Collections.singleton(
				JSONUtil.put("exists", JSONUtil.put("field", "discuss"))));

		ArrayList<JSONObject> should = new ArrayList<>(
			Collections.singleton(
				JSONUtil.put(
					"bool", JSONUtil.put("must_not", mustNot.toString()))));

		ArrayList<JSONObject> clauses = new ArrayList<>(
			Collections.singleton(
				JSONUtil.put(
					"context", "query"
				).put(
					"query",
					JSONUtil.put(
						"bool", JSONUtil.put("should", should.toString()))
				)));

		ArrayList<JSONObject> queryEntries = new ArrayList<>(
			Collections.singleton(JSONUtil.put("clauses", clauses.toString())));
		...
		return JSONUtil.put(
			...
			JSONUtil.put(
				"queryConfiguration",
				JSONUtil.put("queryEntries", queryEntries.toString()))
		).toString(
		).replace(
			StringPool.BACK_SLASH, StringPool.BLANK
		).replace(
			"\"[", "["
		).replace(
			"]\"", "]"
	...
	private void _addCustomSXPElementToSXPBlueprint() throws Exception {
		List<SXPElement> sxpElementList = new ArrayList<>();
		String[] sxpElementNames = {"Custom Element"};
		Object[] configurationValuesArray = {
			HashMapBuilder.<String, Object>put(
				"boost", Integer.valueOf(100)
			).build()
		};

		sxpElementList.add(_sxpElement);
	...
	private static SXPElement _sxpElement;
