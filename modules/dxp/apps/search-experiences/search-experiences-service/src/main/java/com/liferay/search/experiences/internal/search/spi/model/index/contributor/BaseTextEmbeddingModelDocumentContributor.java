...
public abstract class BaseTextEmbeddingModelDocumentContributor
	...
	protected void addLocalizedTextEmbeddings(
		...
		List<String> languageIds = _getLanguageIds(
			(Map<String, Object>)
	...
	private EmbeddingProviderConfiguration _getEmbeddingProviderConfiguration(
		...
		try {
			for (String configurationJSON :
				...
				List<String> assetEntryClassNames =
					(List<String>)attributes.get("assetEntryClassNames");

				if (ListUtil.isEmpty(assetEntryClassNames) &&
					!assetEntryClassNames.contains(clazz.getName())) {

					continue;
				}

				List<String> languageIds = (List<String>)attributes.get(
					"languageIds");

				if (ListUtil.isNotEmpty(languageIds)) {
	...
	private List<String> _getLanguageIds(Map<String, Object> attributes) {
		return (List<String>)attributes.get("languageIds");
	}
