...
import com.liferay.search.experiences.blueprints.engine.internal.condition.visitor.ContainsVisitor;
 ...
 */
@Component(
	immediate = true, property = "name=contains",
	service = ConditionHandler.class
)
public class ContainsConditionHandler implements ConditionHandler {
	...
	public boolean isTrue(
		...
		Function<JSONObject, EvaluationVisitor> function =
			conditionJSONObject -> new ContainsVisitor(
				conditionJSONObject, _messagesHelper);

		return _conditionHelper.evaluate(
			function, false, jsonObject, parameterData, messages);
	}
