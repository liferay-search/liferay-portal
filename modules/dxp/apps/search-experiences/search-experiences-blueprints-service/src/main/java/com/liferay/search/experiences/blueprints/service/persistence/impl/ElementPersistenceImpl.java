...
public class ElementPersistenceImpl
	...
	public void cacheResult(List<Element> elements) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (elements.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public Element updateImpl(Element element) {
		...
		if (isNew && (element.getCreateDate() == null)) {
			if (serviceContext == null) {
				element.setCreateDate(date);
			}
			else {
				element.setCreateDate(serviceContext.getCreateDate(date));
			}
		}

		if (!elementModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				element.setModifiedDate(date);
			}
			else {
				element.setModifiedDate(serviceContext.getModifiedDate(date));
			}
		}
	...
	@Reference
	private ElementModelArgumentsResolver _elementModelArgumentsResolver;
