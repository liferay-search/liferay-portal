...
public class BlueprintPersistenceImpl
	...
	public void cacheResult(List<Blueprint> blueprints) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (blueprints.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public Blueprint updateImpl(Blueprint blueprint) {
		...
		if (isNew && (blueprint.getCreateDate() == null)) {
			if (serviceContext == null) {
				blueprint.setCreateDate(date);
			}
			else {
				blueprint.setCreateDate(serviceContext.getCreateDate(date));
			}
		}

		if (!blueprintModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				blueprint.setModifiedDate(date);
			}
			else {
				blueprint.setModifiedDate(serviceContext.getModifiedDate(date));
			}
		}
	...
	@Reference
	private BlueprintModelArgumentsResolver _blueprintModelArgumentsResolver;
