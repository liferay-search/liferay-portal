...
package com.liferay.portal.search.tuning.blueprints.engine.internal.aggregation.builder;
...
@Component(
	...
	service = AggregationRequestBuilder.class
)
public class TermsAggregationRequestBuilder
	implements AggregationRequestBuilder {
	...
	public Optional<Aggregation> build(
		...
		if (aggregationConfigurationJsonObject.has(
			...
			if (arr.length != 2) {
				searchRequestContext.addMessage(
					new Message(
						...
						TermsAggregationBodyConfigurationKeys.ORDER.
							getJsonKey(),
						orderMetric));
		...
		for (int i = 0; i < childAggregationJsonArray.length(); i++) {
			...
			try {
				AggregationRequestBuilder aggregationBuilder =
					_aggregationRequestBuilderFactory.getBuilder(type);
	...
	private void _setScript(
		...
		if (scriptJsonObject.has(
			...
			if (_log.isWarnEnabled()) {
				_log.warn(
					"Aggregation's script ID or source missing [ " +
						scriptJsonObject + "]");
			}
	...
	private boolean _validate(
		...
		if (Validator.isBlank(name)) {
			searchRequestContext.addMessage(
				new Message(
					Severity.ERROR, "core",
					"core.error.undefined-aggregation-name", null, null, null,
					AggregationConfigurationKeys.NAME.getJsonKey(), null));
			valid = false;

			if (_log.isWarnEnabled()) {
				_log.warn(
					"Aggregation name undefined [ " +
						aggregationConfigurationJsonObject + "]");
			}
		}
		...
		if (!aggregationConfigurationJsonObject.has(
			...
			searchRequestContext.addMessage(
				new Message(
					Severity.ERROR, "core",
					"core.error.undefined-aggregation-field", null, null,
					aggregationConfigurationJsonObject,
					TermsAggregationBodyConfigurationKeys.FIELD.getJsonKey(),
					null));
			valid = false;
	...
	private static final Log _log = LogFactoryUtil.getLog(
		TermsAggregationRequestBuilder.class);

	@Reference
	private AggregationRequestBuilderFactory _aggregationRequestBuilderFactory;
