...
public class StringArrayParameterContributor
	...
	public void contribute(
		...
		if (values == null) {
			Optional<String[]> valueOptional =
				BlueprintJSONUtil.getStringArrayOptional(
					configurationJsonObject,
					CommonRequestParameterConfigurationKeys.DEFAULT.
						getJsonKey());

			if (!valueOptional.isPresent() &&
				(valueOptional.get().length > 0)) {

				return;
			}

			values = valueOptional.get();
		}

		searchParameterData.addParameter(
			new StringArrayParameter(
				parameterName, "${request." + parameterName + "}", values));
	}
