...
package com.liferay.portal.search.tuning.blueprints.engine.impl.internal.executor;
...
import com.liferay.portal.search.tuning.blueprints.constants.json.keys.HighlightConfigurationKeys;
...
public class SearchExecutorImpl implements SearchExecutor {
	...
	public SearchSearchResponse execute(
		...
		if (!searchRequestData.getAggregations(
			).isEmpty()) {
		...
		if (!searchRequestData.getRescores(
			).isEmpty()) {

			searchRequest.setRescores(searchRequestData.getRescores());
		}
		else if (!searchRequestData.getSorts(
				).isEmpty()) {
		...
		if (searchRequestData.getPostFilterQuery(
			).hasClauses()) {
		...
		_setHighlight(searchRequestContext, searchRequest);
	...
	private void _executeQueryPostProcessors(
		...
		for (Map.Entry<String, QueryPostProcessor> entry :
			...
			if (excludedQueryPostProcessorsOptional.filter(
					list -> list.contains(entry.getKey())
				).isPresent()) {
	...
	private void _setHighlight(
		...
		if (Validator.isNotNull(
			...
			String[] fieldNames = JsonUtil.jsonArrayToStringArray(
				fieldNamesJsonArray);
