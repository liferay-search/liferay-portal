...
package com.liferay.portal.search.tuning.blueprints.response.internal;
...
public class BlueprintsResponseBuilderImpl
	...
	protected void registerResponseContributor(
		...
		if (_responseContributors.containsKey(type)) {
			ServiceComponentReference<ResponseContributor> previousReference =
				_responseContributors.get(type);

			if (previousReference.compareTo(serviceComponentReference) < 0) {
				_responseContributors.put(type, serviceComponentReference);
			}
		}
		else {
			_responseContributors.put(type, serviceComponentReference);
	...
	protected void unregisterResponseContributor(
		...
		_responseContributors.remove(type);
	}
