				...
				>
					<c:choose>
						<c:when test="<%= Validator.isNotNull(rankingEntryDisplayContext.getGroupExternalReferenceCode()) %>">
							...
							<span class="lfr-portal-tooltip" data-title='<%= Validator.isNotNull(group) ? HtmlUtil.escape(group.getDescriptiveName(locale)) : LanguageUtil.get(request, "the-site-associated-was-deleted") %>'>
						...
						<c:when test="<%= Validator.isNotNull(rankingEntryDisplayContext.getSXPBlueprintExternalReferenceCode()) %>">
							...
							<span class="lfr-portal-tooltip" data-title='<%= Validator.isNotNull(sxpBlueprint) ? HtmlUtil.escape(sxpBlueprint.getTitle(locale)) : LanguageUtil.get(request, "the-blueprint-associated-was-deleted") %>'>
