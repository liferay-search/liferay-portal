...
public class SearchResultResourceImpl extends BaseSearchResultResourceImpl {
	...
	public Page<SearchResult> postSearchPage(
			String entryClassNames, String siteIds, String search,
		...
		SearchRequestBuilder searchRequestBuilder =
			_searchRequestBuilderFactory.builder(
				...
				searchContext -> _addSearchContextAttributes(
					filter, _csvStringToLongArray(siteIds), search,
					searchContext, searchRequestBody.getAttributes(), sorts)
			);
		...
		String[] documentFieldNames = _getDocumentFieldNames();

		List<String> fields = Arrays.asList(
			ParamUtil.getStringValues(contextHttpServletRequest, "fields"));

		_setSearchRequestFields(
			documentFieldNames, fields, searchRequestBuilder);
		...
		return _toSearchPage(
			searchRequestBody.getFacetConfigurations(),
			Arrays.asList(documentFieldNames), fields, pagination,
	...
	private void _addSearchContextAttributes(
		Filter filter, Long[] siteIds, String search,
		SearchContext searchContext, Map<String, Object> attributes,
		Sort[] sorts) {
		...
		if (ArrayUtil.isNotEmpty(siteIds)) {
			searchContext.setGroupIds(ArrayUtils.toPrimitive(siteIds));
		}
	...
	private Long[] _csvStringToLongArray(String csvString) {
		if (Validator.isBlank(csvString)) {
			return new Long[0];
		}

		String[] stringArray = _csvStringToStringArray(csvString);

		Long[] longArray = new Long[stringArray.length];

		for (int i = 0; i < stringArray.length; i++) {
			longArray[i] = GetterUtil.getLong(stringArray[i]);
		}
	...
	private String[] _getDocumentFieldNames() {
		String nestedFields = ParamUtil.getString(
			contextHttpServletRequest, "nestedFields");

		if (!StringUtil.contains(nestedFields, _DOCUMENT_FIELD_PREFIX, "")) {
			return new String[0];
		}

		List<String> requestedFieldNames = new ArrayList<>();

		ArrayUtil.isNotEmptyForEach(
			_csvStringToStringArray(nestedFields),
			field -> {
				if (field.startsWith(_DOCUMENT_FIELD_PREFIX)) {
					requestedFieldNames.add(
						field.substring(_DOCUMENT_FIELD_PREFIX.length()));
				}
			});

		List<String> restrictFields = Arrays.asList(
			ParamUtil.getStringValues(
				contextHttpServletRequest, "restrictFields"));

		requestedFieldNames.removeAll(restrictFields);

		List<String> restrictedDocumentFieldsList = Arrays.asList(
			_headlessSearchAPICompanyConfiguration.restrictedDocumentFields());

		requestedFieldNames.removeAll(restrictedDocumentFieldsList);

		return requestedFieldNames.toArray(new String[0]);
	}
	...
	private void _includeFields(
		Document document, List<String> documentFieldNames,
		Map<String, Object> fieldsMap) {

		MapUtil.isNotEmptyForEach(
			document.getFields(),
			(fieldName, field) -> {
				if (!_isIncludeField(documentFieldNames, fieldName)) {
					return;
				}
				...
				if (valuesList.size() == 1) {
					fieldsMap.put(fieldName, valuesList.get(0));
				}
				else {
					fieldsMap.put(fieldName, valuesList.toArray());
				}
			});
	}

	private void _includeSourceFields(
		List<String> documentFieldNames, Map<String, Object> fieldsMap,
		SearchHit searchHit) {
	...
	private boolean _isIncludeField(
		List<String> documentFieldNames, String fieldName) {

		if (!ArrayUtil.contains(
				_headlessSearchAPICompanyConfiguration.
					restrictedDocumentFields(),
				fieldName) &&
			documentFieldNames.contains(fieldName)) {
	...
	private void _setSearchRequestFields(
		String[] documentFieldNames, List<String> fields,
		SearchRequestBuilder searchRequestBuilder) {

		if (!ArrayUtil.isEmpty(documentFieldNames)) {
			String[] sourceFieldNames = documentFieldNames;
			...
			if ((fields.isEmpty() || fields.contains("dateModified")) &&
				...
				sourceFieldNames = ArrayUtil.append(
					sourceFieldNames,
					com.liferay.portal.kernel.search.Field.MODIFIED_DATE);
			}

			searchRequestBuilder.fields(sourceFieldNames);
			searchRequestBuilder.fetchSourceIncludes(sourceFieldNames);
		}
	}
	...
	private Map<String, Object> _toDocumentFields(
			List<String> documentFieldNames, SearchHit searchHit)
		throws Exception {

		Map<String, Object> fieldsMap = new HashMap<>();

		_includeFields(searchHit.getDocument(), documentFieldNames, fieldsMap);

		_includeSourceFields(documentFieldNames, fieldsMap, searchHit);
	...
	private SearchPage<SearchResult> _toSearchPage(
			...
			List<String> documentFieldNames, List<String> fields,
		...
		for (SearchHit searchHit : searchHits.getSearchHits()) {
			...
			if (!documentFieldNames.isEmpty()) {
				searchResult.setDocumentFields(
					_toDocumentFields(documentFieldNames, searchHit));
			}
	...
	private static final String _DOCUMENT_FIELD_PREFIX = "documentField.";
